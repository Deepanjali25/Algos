...
public class SpiralTraverseSolutionII {
    ...
    public static void spiralFill(int[][] array,int startRow,int endRow,int startCol,int endCol,ArrayList<Integer> result) {
        ...
        for (int col = endCol - 1; col >=startCol ; col--) {
            //Handle the edge case when there is a single row in the middle of the matrix. In this case, we don't
            //want to double-count the values in this row, which we have already counted in the first for loop
            //above. See test case 8 for an example of this edge case.
            if (startRow == endRow) break;
            result.add(array[endRow][col]);
        }
        for (int row = endRow - 1; row >= startRow + 1 ; row++) {
            //Handle the edge case when there is a single column in the middle of the matrix. In this case, we
            // don't want to double-count the values in this column, which we have already counted in the second
